...
//	rawFile = open( filename.c_str(), O_WRONLY | O_CREAT | O_TRUNC, S_IWUSR );
	rawFile = open( filename.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644 );
...
bool Bitmap::save(const unsigned char *data, const std::vector<unsigned char>& p_rAdditionalData)
{
	...
	header_.fileSize	+=	p_rAdditionalData.size();
	header_.reserved1	=	p_rAdditionalData.size() != 0 ? 1971 : 0;
	header_.reserved2	=	p_rAdditionalData.size();
	header_.dataOffset	+=	p_rAdditionalData.size();
