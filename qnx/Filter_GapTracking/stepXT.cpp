 ...
 * 	@date		2018
 * 	@brief 		This filter detects a step in the laser-line. New inPipes "Heigth" and "Angel" instead of Parameters
 */
...
//#include "filter/calibrationSingleton.h"
...
const std::string StepXT::m_oFilterName = std::string("StepXT");
const std::string StepXT::PIPENAME_IN1 = std::string("Angle");
const std::string StepXT::PIPENAME_IN2 = std::string("Height");
const std::string StepXT::PIPENAME_OUT1 = std::string("PositionX");
const std::string StepXT::PIPENAME_OUT2 = std::string("PositionY");


StepXT::StepXT() :
	TransformFilter(StepXT::m_oFilterName, Poco::UUID{"2697b537-ea3d-4045-926d-71a20ecaca7b"}),
	m_pPipeInLaserLine(nullptr),
	m_oPipeInAngle(nullptr),
	m_oPipeInHeight(nullptr),
	m_oPipePositionXOut(this, StepXT::PIPENAME_OUT1),
	m_oPipePositionYOut(this, StepXT::PIPENAME_OUT2),
	m_oLeft(true),
	//m_oHeight( 1.5 ),
	m_oHeightTolNegative(0.5),
	m_oHeightTolPositive(0.5),
	m_oLengthLeft(20.0),
	m_oLengthRight(20.0),
	m_oAngle(10.0),
	m_oTypeOfLaserLine(LaserLine::FrontLaserLine)
{
	...
	parameters_.add("HeightTolNeg", Parameter::TYPE_double, m_oHeightTolNegative);
	parameters_.add("HeightTolPos", Parameter::TYPE_double, m_oHeightTolPositive);
    ...
    setInPipeConnectors({{Poco::UUID("43fcfa71-0cca-4e2e-8b52-c2ff9e32862a"), m_pPipeInLaserLine, "LaserLine", 1, "LaserLine"},
    {Poco::UUID("0c60aa62-b74d-4c56-985c-33ceb789d8bf"), m_oPipeInAngle, "Angle", 1, "Angle"},
    {Poco::UUID("2284197f-eb70-4b3e-8bbd-4ccb4f39313d"), m_oPipeInHeight, "Height", 1, "Height"}});
    setOutPipeConnectors({{Poco::UUID("34f8984b-80d3-458c-941c-aa67e842b382"), &m_oPipePositionXOut, "PositionX", 0, ""},
    {Poco::UUID("6aa67a5a-d150-4a09-9098-50715c8449f9"), &m_oPipePositionYOut, "PositionY", 0, ""}});
    setVariantID(Poco::UUID("83a1fb47-087e-43c4-b818-c9d00d1e9c8d"));
} // CTor.
...
void StepXT::setParameter()
{
	...
	m_oHeightTolNegative = parameters_.getParameter("HeightTolNeg").convert<double>();
	m_oHeightTolPositive = parameters_.getParameter("HeightTolPos").convert<double>();
...
bool StepXT::subscribe(fliplib::BasePipe& p_rPipe, int p_oGroup)
{
	...
	if (p_rPipe.tag() == "Angle") {
		m_oPipeInAngle = dynamic_cast<fliplib::SynchronePipe < interface::GeoDoublearray > *>(&p_rPipe);
	} // if
	if (p_rPipe.tag() == "Height") {
		m_oPipeInHeight = dynamic_cast<fliplib::SynchronePipe < interface::GeoDoublearray > *>(&p_rPipe);
	} // if
...
void StepXT::proceedGroup(const void* p_pSender, fliplib::PipeGroupEventArgs& p_rE)
{
	poco_assert_dbg(m_pPipeInLaserLine != nullptr); 	// to be asserted by graph editor
	poco_assert_dbg(m_oPipeInAngle != nullptr); // to be asserted by graph editor
	poco_assert_dbg(m_oPipeInHeight != nullptr); // to be asserted by graph editor

	// data Angle and Height
	const interface::GeoDoublearray &rGeoDoubleArrayInAngle = m_oPipeInAngle->read(m_oCounter);
	const interface::GeoDoublearray &rGeoDoubleArrayInHeight = m_oPipeInHeight->read(m_oCounter);

	unsigned int sizeOfArrayAngle = rGeoDoubleArrayInAngle.ref().size();
	unsigned int sizeOfArrayHeight = rGeoDoubleArrayInHeight.ref().size();

	bool hasDoubleInputs = (sizeOfArrayAngle > 0) && (sizeOfArrayHeight > 0); // Neu: Ueberpruefung, ob ein Eingang eine leere Liste ist

	if (sizeOfArrayAngle != 1) { // result is always one point
		wmLog(eDebug, "Filter '%s': Received %u Angle values. Can only process first element, rest will be discarded.\n", m_oFilterName.c_str(), sizeOfArrayAngle);
	}
	if (sizeOfArrayHeight != 1) { // result is always one point
		wmLog(eDebug, "Filter '%s': Received %u Height values. Can only process first element, rest will be discarded.\n", m_oFilterName.c_str(), sizeOfArrayHeight);
	}
	double oAngleDegrees = rGeoDoubleArrayInAngle.ref().getData().front();
	m_oAngle = M_PI / 180.0 * oAngleDegrees;
	m_oHeight = rGeoDoubleArrayInHeight.ref().getData().front();
	...
	//check height tolerance negative
	if (m_oHeight < m_oHeightTolNegative) {
		double negTolAbs = m_oHeight - m_oHeightTolNegative;
		m_oHeightTolNegative = m_oHeightTolNegative + negTolAbs;
	}
	...
	if (inputIsInvalid(rLaserLine) || !hasDoubleInputs)
	{
...
StepXT::StepCandidate StepXT::findStep( const geo2d::Doublearray &p_rLineIn )
{
	...
	for( unsigned int oIndexLine = m_oFeatureCenter + 2; oIndexLine < rLineData.size() - ( m_oFeatureCenter + m_oLengthRight + 2); ++oIndexLine ) {
		...
		for ( unsigned int oIndexFeat = 0; oIndexFeat < m_oFeature.size(); ++oIndexFeat ) {
			if ( rLineRank[oIndexLine + (oIndexFeat - m_oFeatureCenter)] > eRankMin ) {
				oLineAvg += rLineData[oIndexLine + (oIndexFeat - m_oFeatureCenter)];
				oLineAvgCnt++;
			}
		} // for
		oLineAvg /= oLineAvgCnt;

		auto oInvalidCnt 	= (unsigned int)(0);
		for ( unsigned int oIndexFeat = 0; oIndexFeat < m_oFeature.size(); ++oIndexFeat ) {
			if ( rLineRank[oIndexLine + (oIndexFeat - m_oFeatureCenter)] > eRankMin ) {
				oScore  += ( std::abs( rLineData[oIndexLine + (oIndexFeat - m_oFeatureCenter)] - ( m_oFeature[oIndexFeat] + oLineAvg ) ) ) * ( std::abs( rLineData[oIndexLine + (oIndexFeat - m_oFeatureCenter)] - ( m_oFeature[oIndexFeat] + oLineAvg ) ) );
			} else {
				oInvalidCnt++;
	...
	for ( unsigned int oIndex = 0; oIndex < m_oCandidates.size(); ++oIndex ) {
		if ( std::get<eStepValid>(m_oCandidates[oIndex]) == true ) {
			...
			if (std::get<eStepHeight>(m_oCandidates[oIndex]) < (m_oHeight - m_oHeightTolNegative)
				|| std::get<eStepHeight>(m_oCandidates[oIndex]) > (m_oHeight + m_oHeightTolPositive))
			{
				std::get<eStepValid>(m_oCandidates[oIndex]) = false;
			} // if
		} // if
...
void StepXT::findSteps( const geo2d::VecDoublearray &p_rLineIn, geo2d::Pointarray &p_rPositionOut )
{
...
void StepXT::calcHeight( const geo2d::Doublearray &p_rLineIn, StepCandidate& p_rStep )
{
...
void StepXT::lineFit( const geo2d::Doublearray &p_rLineIn, unsigned int p_oMinIndex, unsigned int p_oMaxIndex, double& p_rMOut, double& p_rTOut )
{
	auto& rLineData 	= p_rLineIn.getData();
	auto& rLineRank = p_rLineIn.getRank();
