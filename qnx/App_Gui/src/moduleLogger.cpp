 ...
 *  @date       2019
 *  @brief      Essentially the same as the basemodule logger, but as the GUI is not really a WM module, I was not able to use the standard logger.
 */
...
extern std::unique_ptr<precitec::ModuleLogger> g_pLogger;
...
{
    return g_pLogger->getMessage();
}


void invokeIncreaseWriteIndex()
{
    g_pLogger->increaseWriteIndex();
}

void redirectLogMessage(UNUSED LogMessage *msg)
{
}
