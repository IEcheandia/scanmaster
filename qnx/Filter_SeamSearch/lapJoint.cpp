...
const std::string LapJoint::m_oFilterName 		= std::string("LapJoint");
const std::string LapJoint::PIPENAME_SEAMPOS_OUT	= std::string("SeamPositionOut");
const std::string LapJoint::PIPENAME_SEAMLEFT_OUT	= std::string("SeamLeft");
const std::string LapJoint::PIPENAME_SEAMRIGHT_OUT	= std::string("SeamRight");
const std::string LapJoint::PIPENAME_AREALEFT_OUT	= std::string("AreaLeft");
const std::string LapJoint::PIPENAME_AREARIGHT_OUT	= std::string("AreaRight");

LapJoint::LapJoint() :
	TransformFilter( LapJoint::m_oFilterName, Poco::UUID{"0AF4713E-8E1A-4CFA-87E9-40DB668AFCA9"} ),
	...
	m_oMaxLineDiff(1)
{
	m_pPipeOutSeamPos = new SynchronePipe< interface::GeoDoublearray > ( this, LapJoint::PIPENAME_SEAMPOS_OUT );
	m_pPipeOutSeamLeft = new SynchronePipe< interface::GeoDoublearray > ( this, LapJoint::PIPENAME_SEAMLEFT_OUT );
	m_pPipeOutSeamRight = new SynchronePipe< interface::GeoDoublearray > ( this, LapJoint::PIPENAME_SEAMRIGHT_OUT );
	m_pPipeOutAreaLeft = new SynchronePipe< interface::GeoDoublearray > ( this, LapJoint::PIPENAME_AREALEFT_OUT );
	m_pPipeOutAreaRight = new SynchronePipe< interface::GeoDoublearray > ( this, LapJoint::PIPENAME_AREARIGHT_OUT );
    ...
    setInPipeConnectors({{Poco::UUID("91D74CB4-CE33-49DA-A65A-0C6708BC068C"), m_pPipeInImageFrame, "ImageFrameIn", 1, "ImageFrame"},
    {Poco::UUID("EEB1DA23-B8FA-4B86-B2CB-C22704FAD4BB"), m_pPipeInLaserLine, "LaserLine", 1, "LaserLine"},
    {Poco::UUID("BC64EF6C-B20B-4A2B-9FC1-7E7DE775B658"), m_pPipeInLeftLineSlope, "LeftSlopeIn", 1, "LeftSlope"},
    {Poco::UUID("ED321C7F-13B8-4FF6-AD33-E4B7F14C1755"), m_pPipeInLeftLineYIntercept, "LeftYInterceptIn", 1, "LeftYIntercept"},
    {Poco::UUID("55C3A9A8-1614-4924-8177-E2624A8BD474"), m_pPipeInRightLineSlope, "RightSlopeIn", 1, "RightSlope"},
    {Poco::UUID("53627736-723C-4E12-A4EB-0ACE853B3ED9"), m_pPipeInRightLineYIntercept, "RightYInterceptIn", 1, "RightYIntercept"}});

    setOutPipeConnectors({{Poco::UUID("B46943FD-55E6-404F-9F5F-C00FB98CBBB2"), m_pPipeOutSeamPos, PIPENAME_SEAMPOS_OUT, 0, ""},
    {Poco::UUID("3A60F4D9-110F-4300-8584-5C89C0E78462"), m_pPipeOutSeamLeft, PIPENAME_SEAMLEFT_OUT, 0, ""},
    {Poco::UUID("3C4DB4E5-B38B-4E9A-B648-6DBEC13FA34C"), m_pPipeOutSeamRight, PIPENAME_SEAMRIGHT_OUT, 0, ""},
    {Poco::UUID("626B10BB-B65C-4261-8C52-6DDD2CBD7A00"), m_pPipeOutAreaLeft, PIPENAME_AREALEFT_OUT, 0, ""},
    {Poco::UUID("1A4605E0-4424-4C2E-8CC3-E947C5F1A22D"), m_pPipeOutAreaRight, PIPENAME_AREARIGHT_OUT, 0, ""}});
    setVariantID(Poco::UUID("26EF10F8-832E-468D-B2D9-5EADE642835A"));
} // LineProfile
...
bool LapJoint::subscribe(fliplib::BasePipe& p_rPipe, int p_oGroup)
{
...
void LapJoint::proceedGroup(const void* p_pSender, fliplib::PipeGroupEventArgs& p_rE)
{
...
void LapJoint::setModeVariables()
{
	switch (m_oMode)
	{
	case 0:
...
void LapJoint::calcSeam( const image::BImage &p_rImageIn, const geo2d::VecDoublearray &p_rLaserLineIn,
		...
		{ // loop over N lines
			...
			if (m_doLeft)
			{
				for (int x = m_seamStartX; x < m_seamEndX; x++) // von links nach rechts
				{
					...
					if (m_isTracking)
					{
						if ((curDiff <= m_oMaxLineDiff) && !leftFound) // Abstand zu gering
						{
							diffSumLeft = 0;
						}
						else
					...
					{
						if ((std::abs(curDiff) <= m_oMaxLineDiff) && !leftFound) // Abstand zu gering
						{
							diffSumLeft = 0;
						}
			...
			if (m_doRight)
			{
				for (int x = m_seamEndX - 1; x > m_seamStartX; x--)  // von rechts nach links
				{
					...
					if (m_isTracking)
					{
						if ((curDiff <= m_oMaxLineDiff) && !rightFound) // Abstand zu gering
						{
							diffSumRight = 0;
						}
						else
					...
					{
						if ((std::abs(curDiff) <= m_oMaxLineDiff) && !rightFound) // Abstand zu gering
						{
							diffSumRight = 0;
						}
...
//void LapJoint::calcParabel(const image::BImage &p_rImageIn, const geo2d::VecDoublearray &p_rLaserLineIn, geo2d::Doublearray seamLeft, geo2d::Doublearray seamRight,
...
//			const auto& rLaserLineIn_Rank = p_rLaserLineIn[lineN].getRank();
//
//			m_seamStartX = seamLeft.getData()[lineN];
//			m_seamEndX   = seamRight.getData()[lineN];
//
...
//
//			for (unsigned int x = m_seamStartX; x < m_seamEndX; x++) // von links nach rechts
//			{
//				lineY = (int)rLaserLineIn_Data[x]; // Y-Wert aus Tracking
//				rank = (int)rLaserLineIn_Rank[x];
//
...
//}
//
//void ParabelFit::AddPoint(double x, double y)
//{
//	iPointAnz++;
//
//	S01 += y;
...
//}
//
//void ParabelFit::DelPoint(double x, double y)
//{
...
//}
//
//bool ParabelFit::CalcABC(double & a, double & b, double & c)
//{
//	a = b = c = 0.0;
//
